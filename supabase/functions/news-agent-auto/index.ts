  ...
  try {
    ...
    for (const target of localTargets) {
      ...
      const localFiltered = localRss.filter((item) => {
        ...
        return blob.includes(target.city) || blob.includes(target.country);
      });
      const localPool = localFiltered.length ? localFiltered : localRss;
      for (const item of localPool.slice(0, 8)) {
        if (!item.title || !item.link) continue;
        if (!isLiveMintItem(item)) continue;
        const bodyText = `${item.title}. ${item.description || ""}`;
        const cleanedTitle = stripSourceTail(stripUrlsAndDomains(stripHtml(item.title)));
    ...
    if (localRows.length > 0) {
      const localKeys = Array.from(new Set(localRows.map((r) => `${r.city || ""}|${r.country || ""}`)));
    ...
    return new Response(JSON.stringify({ ok: true, inserted: dedupedPayload.length }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
